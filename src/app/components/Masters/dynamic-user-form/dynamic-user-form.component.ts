...
interface Department { id: number; department_name: string; org_id: number; org_name: string; }
...
@Component({
  ...
  styleUrl: './dynamic-user-form.component.css'
})
export class DynamicUserFormComponent {
  ...
  //   console.log("inside first", this.registerForm.get('department_id')?.value);
  //   if (!event.checked) {
  //      this.orgDepartments.clear();
  //     this.registerForm.get('multiple_org_ids')?.reset();
  //     this.registerForm.get('department_id')?.reset();
  ...
  onMultipleOrgToggle(event: any) {
  if (!event.checked) {
    // switched TO SINGLE ORG
    this.orgDepartments.clear();
    this.registerForm.get('multiple_org_ids')?.reset();

    // ensure department_id control exists
    if (!this.registerForm.get('department_id')) {
      this.registerForm.addControl('department_id', this.fb.control('', Validators.required));
    } else {
      this.registerForm.get('department_id')?.reset();
    }
  ...
  onMultipleOrganizationChange(selectedIds: any[]) {
    ...
    console.log(this.organizationsarray, 'here is org array');
  ...
  closeDialog() {
    ...
    // Reset the FormArray to empty state
    this.orgDepartments.reset();
    this.orgDepartments.clear();
    this.incominDepartmentData = []
    this.registerForm.get('isMultipleOrg')?.reset();
        this.registerForm.get('department_id')?.reset();
  ...
  addUser() {
    ...
    if (formValue.isMultipleOrg) {
      // Use orgDepartments FormArray - same logic as getRoles()
      ...
      if (orgDepartmentsArr && orgDepartmentsArr.length > 0) {
        orgDepartmentsArr.controls.forEach((group: any) => {
          orgArray.push({
            ...
            department_id: group.get('department_id')?.value || null
          });
        });
      }
    } else {
  ...
  removeOrganization(index: number): void {
    ...
    this.orgDepartments.removeAt(index);
          ...
          if (res.status == 200) {
            ...
            (res.data || []).forEach((org: any) => {
              (org.department || []).forEach((dept: any) => {
                this.incominDepartmentData.push({
                  id: dept.id,
            ...
            console.log(this.incominDepartmentData, 'here departments array');
...
handleSingleOrg(orgId: number) {
  const org = this.organizationsarray.find(o => o.id === orgId);
  this.departmentsarray = org?.department ?? [];
  ...
  getDepartmentsForOrg(orgId: number) {
    const organization = this.organizationsarray.find(org => org.id === orgId);
  ...
  getRoles() {
    ...
    if (this.registerForm.get('isMultipleOrg')?.value) {
      ...
      console.log('here in the single org selection came]',this.departmentsarray);
      
      let org_id = this.registerForm.get('org_id')?.value || null;
      let department_id = this.registerForm.get('department_id')?.value || null;

            console.log('here in the single org selection came',org_id);

      console.log('here in the single org selection came',department_id);
      orgPayload.push({
        org_id: org_id,
        department_id: department_id
      });
    ...
    this.designationsarray = []
    this.commonService.postDataWithBody('api/user/getRolesMappedWithDepartmentandOrg', { org: orgPayload }).subscribe({
      next: (res: any) => {
        if (res.status == 200 || res.status == 201) {
          this.designationsarray = res.data;
          if(this.designationsarray.length === 1){
            this.registerForm.get('designation_id')?.setValue(this.designationsarray[0].user_role_id)
          }
