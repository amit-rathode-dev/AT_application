...
export function roleExistsValidator(existingFinance: { id: number; name: string }[]): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    if (!control.value || !existingFinance?.length) return null;

    const input = control.value.trim().toLowerCase();

    const exists = existingFinance.some(itr => itr.name.trim().toLowerCase() === input);
...
@Component({
 ...
 imports: [CommonModule, ReusablemodulesComponent, DropdownModule, TableModule, DialogModule, ReactiveFormsModule, ReusablemodulesComponent, PaginatorModule, NoDataPipe],
  ...
  templateUrl: './finance-org.component.html',
  styleUrl: './finance-org.component.css'
})
export class FinanceOrgComponent {
  ...
  applyRoleNameValidators(): void {
    const nameControl = this.financeOrgForm.get('name');
    if (!nameControl) return;
    nameControl.setValidators([
      Validators.required,
      roleExistsValidator(this.financeOrgData)
    ]);
