...
// //   const user = JSON.parse(sessionStorage.getItem('userDetails') || 'null');
...
//   const encryptedUser = sessionStorage.getItem('userDetails');
...
export const roleGuard: CanActivateFn = (route, state) => {
  ...
  // Decrypt and parse stored user details
  const decryptedUser = encryptionService.getItem<any>('userDetails'); 

  const allowedRoles = route.data?.['role_name'] as string[] | undefined;
  const allowedOrgType = route.data?.['org_type'] as string | undefined;

  const userRole = decryptedUser?.role_name;
  const orgType = decryptedUser?.org_type;
